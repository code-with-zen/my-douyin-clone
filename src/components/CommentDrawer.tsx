...
import { Avatar, SideSheet, Toast } from '@douyinfe/semi-ui';
  ...
  return (
    <SideSheet
      title={
        <span className="text-white text-[16px] font-bold">
          全部评论 ({commentCount + comments.length - INITIAL_COMMENTS.length})
        </span>
      }
      visible={visible}
      onCancel={onClose}
    ...
    >
        ...
        {comments.map(item => (
          <div key={item.id} className="flex gap-3 group">
            ...
            <div className="flex-1 flex flex-col gap-1">
              ...
              <span className="text-white text-[14px] leading-6 break-all">
                {item.content}
              </span>
            ...
            <div
              className="flex flex-col items-center gap-1 cursor-pointer w-[30px]"
      ...
      <div className="absolute bottom-0 left-0 w-full px-4 py-3 bg-[#161823] border-t border-[#333] flex items-center gap-2">
        ...
        <button
          onClick={handleSend}
          className={`text-sm font-bold transition-colors ${inputValue ? 'text-[#fe2C55] cursor-pointer' : 'text-gray-600 cursor-not-allowed'}`}
        >
          发布
        </button>
      </div>
