...
import { IconTiktokLogo, IconHome, IconLive, IconUser, IconStar } from "@douyinfe/semi-icons";
import "./index.css";
    ...
    <Layout className="h-screen w-screen overflow-hidden bg-black flex flex-col text-white">
      <Header className="h-[60px] shrink-0 bg-[#161823] border-b border-[#333] flex items-center px-4 z-10 relative">
        ...
        <div className="ml-auto mr-2">
          <Button theme="solid" type="primary" style={{ backgroundColor: '#fe2C55', borderColor: '#fe2C55' }}>
            登录
          </Button>
        </div>
      </Header>
          ...
          {MENU_ITEMS.map((item) => {
            ...
            return (
              <div
                ...
                `}
              >
                <div className={`text-[24px] leading-none mb-[2px] transition-transform duration-200 ${isActive ? "scale-105 text-[#fe2C55]" : ""}`}>
                  {item.icon}
                </div>
                <span className={`w-full flex items-center justify-center text-[16px] leading-none whitespace-nowrap origin-top transform ${isActive ? "font-bold" : ""}`}>
                  {item.text}
                </span>
              </div>
            );
